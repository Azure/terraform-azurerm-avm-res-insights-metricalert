...
- **Custom properties are only accepted on `Query` kind rules.** Setting `properties.customProperties` on a metric criteria rule is rejected with `CustomProperties are currently supported for 'Query' kind Metric Alert rule only`. Because this module implements the metric and web test criteria models, the property is unusable on every configuration the module can produce, so no `custom_properties` input is exposed. It becomes applicable only if PromQL/query criteria are added.
